...
class PremiumShopAdapter(
    ...
    private class ItemVH(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(
            ...
            status.text = when {
                row.selected -> safeStr(ctx, R.string.premium_selected, "Selecionado")
                row.unlocked -> safeStr(ctx, R.string.premium_unlocked, "Desbloqueado")
                else -> item.statusTextSafe(ctx)
            }

            // limpa listeners
            mainBtn.setOnClickListener(null)
            petUpBtn.setOnClickListener(null)
            ...
            when {
                !row.unlocked && item.priceCoins > 0 -> {
                    ...
                    mainBtn.text = if (pode)
                        safeStr(ctx, R.string.premium_btn_buy, "Comprar (${item.priceCoins})", item.priceCoins)
                    else
                        safeStr(ctx, R.string.premium_btn_missing, "Faltam $falta", falta)
                    ...
                    if (pode) {
                        mainBtn.setOnClickListener {
                            ...
                            Toast.makeText(
                                ctx,
                                safeStr(ctx, R.string.premium_toast_bought_applied, "Comprado e aplicado ✅"),
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
                ...
                !row.unlocked && item.priceCoins <= 0 -> {
                    mainBtn.text = safeStr(ctx, R.string.premium_btn_achievement, "Por conquista")
                ...
                row.unlocked && row.selected -> {
                    mainBtn.text = safeStr(ctx, R.string.premium_btn_applied, "Aplicado")
                    mainBtn.isEnabled = false
                    mainBtn.alpha = 0.85f
                ...
                row.unlocked && !row.selected -> {
                    mainBtn.text = safeStr(ctx, R.string.premium_btn_apply, "Aplicar")
                    ...
                    mainBtn.setOnClickListener {
                        ...
                        Toast.makeText(
                            ctx,
                            safeStr(ctx, R.string.premium_toast_applied, "Aplicado ✅"),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
            ...
            if (item.type == PremiumType.PET && row.unlocked && item.id != "pet_none") {
                ...
                petLevelTv.text = safeStr(ctx, R.string.premium_pet_level, "Nível: $lvl", lvl)
                ...
                if (lvl >= 3) {
                    ...
                    val coinsNow = CoinManager.getCoins(ctx)
                    val pode = coinsNow >= cost

                    petUpBtn.text = if (pode)
                        safeStr(ctx, R.string.premium_btn_upgrade, "Upar p/ $next ($cost)", next, cost)
                    else
                        safeStr(ctx, R.string.premium_btn_missing, "Faltam ${(cost - coinsNow).coerceAtLeast(0)}",
                            (cost - coinsNow).coerceAtLeast(0))
                    ...
                    petUpBtn.setOnClickListener {
                        ...
                        Toast.makeText(
                            ctx,
                            safeStr(ctx, R.string.premium_toast_pet_upgraded, "Pet evoluído ✅"),
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
        ...
        private fun defaultDesc(ctx: Context, item: PremiumItem): String = when (item.type) {
            PremiumType.THEME -> safeStr(ctx, R.string.premium_desc_theme, "Muda o visual do app")
            PremiumType.FRAME -> safeStr(ctx, R.string.premium_desc_frame, "Moldura do avatar")
            PremiumType.TITLE -> safeStr(ctx, R.string.premium_desc_title, "Título exibido no perfil")
            PremiumType.PET -> safeStr(ctx, R.string.premium_desc_pet, "Mascote animado")
            PremiumType.VFX -> safeStr(ctx, R.string.premium_desc_vfx, "Efeito de vitória")
        }
        ...
        private fun PremiumItem.statusTextSafe(ctx: Context): String {
            return runCatching { this.statusText(ctx) }.getOrElse {
                if (priceCoins > 0) safeStr(ctx, R.string.premium_status_locked_coins, "🔒 $priceCoins moedas", priceCoins)
                else safeStr(ctx, R.string.premium_status_achievement, "🏆 Conquista", safeStr(ctx, R.string.premium_req_none, "Sem requisito"))
            }
        }

        private fun safeStr(ctx: Context, resId: Int, fallback: String, vararg args: Any): String {
            return try {
